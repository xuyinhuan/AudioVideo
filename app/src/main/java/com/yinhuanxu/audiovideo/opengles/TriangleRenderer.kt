...
class TriangleRenderer : GLSurfaceView.Renderer {
    ...
    // 顶点着色器的脚本
    private val verticesShader = ("attribute vec2 vPosition;            \n" // 顶点位置属性vPosition
            + "void main(){                         \n"
     ...
     */
    override fun onSurfaceCreated(gl10: GL10?, eglConfig: EGLConfig?) { // 初始化着色器
        // 基于顶点着色器与片元着色器创建程序
     ...
     */
    override fun onSurfaceChanged(gl10: GL10?, width: Int, height: Int) { // 设置绘图的窗口(可以理解成在画布上划出一块区域来画图)
     ...
     * @param gl10 同onSurfaceCreated()
     */
    override fun onDrawFrame(gl10: GL10?) { // 获取图形的顶点坐标
        val vertices: FloatBuffer = getVertices()
        // 清屏
     ...
     * @return shader索引
     */
    private fun loadShader(shaderType: Int, sourceCode: String): Int { // 创建一个新shader
        var shader = GLES20.glCreateShader(shaderType)
        // 若创建成功则加载shader
        if (shader != 0) { // 加载shader的源代码
